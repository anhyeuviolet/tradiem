...
if(!defined('NV_IS_DSCB_ADMIN'))
{
	...
	if($delid !=0){
		...
		$db->query("DELETE FROM" . NV_PREFIXLANG . "_" . $module_data . " WHERE gvid='".$delid."'");
	}//End IF
	if ($tcid > 0){
    $sql = "SELECT DISTINCT gvid, hoten, ngsinh, active,sorttc FROM " . NV_PREFIXLANG . "_" . $module_data . " WHERE tcid= '". $tcid ."' ORDER BY sorttc ASC";
	$result = $db->query( $sql ) or die ('Đã có lỗi xảy ra trong lệnh truy vấn CSDL.<br />');
    $num = $result->rowCount();
	}
	if($num > 0){
		$contents .= "<br /><table summary=\"\" class=\"table\">\n";
		...
		$resulttc = $db->query( $sqltc);
		while ($dstc = $resulttc->fetch())
		{
			if ($tcid==$dstc[0]){
				$tentc = $dstc[1];
				$sel = "selected";
