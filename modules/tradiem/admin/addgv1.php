...
if ($nv_Request->get_int ( 'save', 'post' )) {
	...
	if (! empty ( $id )) {
		$result = $db->query ( "UPDATE " . NV_PREFIXLANG . "_" . $module_data . "_dsgv SET tengv=" . $db->quote ( $tengv ). ",user=" . $db->quote ( $user ). " WHERE gvid=" . $id . "" );
		...
		$alreadygvid = $db->sql_numrows ( $db->query ( "SELECT gvid FROM " . NV_PREFIXLANG . "_" . $module_data . "_dsgv WHERE gvid = " . $db->quote ( $gvid ) . "" ) );
		if (! $alreadygvid) {
			$result = $db->query ( "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_dsgv (gvid,tengv,user)VALUES (" . $db->quote ( $gvid ) . ", " . $db->quote ( $tengv ) . ", " . $db->quote ( $user ) . ")" );
	...
	if (! empty ( $id )) {
		$row = $db->fetch ( $db->query ( "SELECT * FROM " . NV_PREFIXLANG . "_" . $module_data . "_dsgv WHERE gvid='$id'" ) );
