...
$impdsgv = $nv_Request->get_title ( 'impdsgv', 'post','', 1);
...
//
	   ...
	   if( $dscb[0]!="" and $dscb[1]!="") {
		...
		if(!$numrows) {
	   	$sql1 = "INSERT INTO " . NV_PREFIXLANG . "_" . $module_data . "_dsgv (gvid, tengv, user, log, chunhiem, active) VALUES ('".$dscb[0]."', '".$dscb[1]."', '".$dscb[2]."', '".$dscb[3]."', '".$dscb[4]."', '".$dscb[5]."')";
			$result = $db->query($sql1) or die ('Đã có lỗi xảy ra trong quá trình thêm danh sách giáo viên.<br />');
			$them=$them+1;
		}else{
		...
		$db->query($sql2) or die ('Đã có lỗi xảy ra trong quá trình cập nhật danh sách giáo viên.<br />');
		$sua=$sua+1;
		}
		}
		}  
	...
	$contents .= "<table class=\"table\" style='width:500px'>\n";
	$contents .= "<thead>\n";
	$contents .= "<tr>\n";
